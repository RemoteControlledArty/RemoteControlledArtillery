...
if (_ammo find 'HEAB' > -1) then {	\
	[_projectile, _gunner] spawn {	\
		...
		waitUntil {	\
			(count ((nearestObjects [_projectile, ['Man','Car','Air','Truck','Tank','laserTarget','Motorcycle'], 10]) select {alive _x && side _x != _gunnerSide && _gunner knowsAbout _x > 0})) > 0;	\
		};	\
		triggerAmmo _projectile;	\
		hint format ['airbursting %1', _projectile];	\
		sleep 2;	\
		hint format ['%1',''];	\
	};	\
};";