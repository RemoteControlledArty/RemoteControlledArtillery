...
[_vehicle, _targetPosition, _timeSinceLast, _ETA, _proj] spawn {
    ...
    private _firstUnit = (units resistance) select 0;
    ...
    private _targetGrid = mapGridPosition _targetPosition;
    ...
    [_proj, _shownETA] remoteExec ['RC_fnc_RC_CBRad_projArrRE', resistance];
    ...
    //if (_ETA > RC_CBRad_DetectionTime_P) then {
        ...
        if ((_targetPosition select 0) == 0) then {
            ...
            private _nearPlayers = allPlayers select {_x distance2D _targetPosition < 200};
            private _nearPlayers_I = _nearPlayers select {side _x == resistance};
    ...
    if (_timeSinceLast > RC_CBRad_IgnoreTime_P) then {
        ...
        private _roundedArtySourcePosX = round (_artySourcePos select 0);
        private _roundedArtySourcePosY = round (_artySourcePos select 1);
        ...
        private _artySourceMarkerHour = date select 3;
        private _artySourceMarkerMinute = date select 4;
