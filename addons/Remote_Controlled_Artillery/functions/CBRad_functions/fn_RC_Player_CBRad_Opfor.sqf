...
[_vehicle, _targetPosition, _timeSinceLast, _ETA, _proj] spawn {
    ...
    private _firstUnit = (units east) select 0;
    ...
    [_proj, _shownETA] remoteExec ['RC_fnc_RC_CBRad_projArrRE', east];
    ...
    //if (_ETA > RC_CBRad_DetectionTime_P) then {
        ...
        if ((_targetPosition select 0) == 0) then {
            ...
            private _nearPlayers_O = _nearPlayers select {side _x == east};
