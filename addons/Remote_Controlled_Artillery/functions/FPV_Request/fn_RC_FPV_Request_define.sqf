...
fnc_RC_FPV_Request_Search2 = {
	...
	if (isNull _nearest) then {
		...
		[_veh, _weapon, _uavClass, _sidePlayer, _flyToPos] call fnc_RC_FPV_Request_deploy2;
	};
};
	...
	//forEach _validVeh;
	{
		...
		*/
		...
		if (_idx > -1 && _continue) exitWith {
			...
			//output
			[_veh, _classToRemove, _idx, _counts, _newCount]
		...
		if (_idx2 > -1 && _continue) exitWith {
			...
			*/
			...
			//output
			[_veh, _classToRemove, _idx2, _counts, _newCount]
