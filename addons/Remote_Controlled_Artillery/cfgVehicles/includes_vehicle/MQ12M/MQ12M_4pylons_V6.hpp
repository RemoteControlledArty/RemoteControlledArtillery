/*
class TransportPylonsComponent: TransportPylonsComponent
{
	class pylons: pylons
	{
		//Right outer
		class pylon1: pylon1
		{
			hardpoints[]=
			{
				"RC_Hardpoint_FuelTank",

				"RC_Hardpoint_V1",
				"RC_Hardpoint_V1_Right",
				"RC_Hardpoint_V2",
				"RC_Hardpoint_V2_Right",
				"RC_Hardpoint_V3",
				"RC_Hardpoint_V3_Right",
				"RC_Hardpoint_V4",
				"RC_Hardpoint_V4_Right",
				//"RC_Hardpoint_V5",
				//"RC_Hardpoint_V5_Right",

				//"RC_Hardpoint_V6",
				//"RC_Hardpoint_V6_Right"

				"RC_Hardpoint_V5_Green"
			};

			attachment="RC_PylonM_12Rnd_SRAM_Green";
		};
		//Right inner
		class pylon2: pylon2
		{
			hardpoints[]=
			{
				"RC_Hardpoint_FuelTank",

				"RC_Hardpoint_V1",
				"RC_Hardpoint_V1_Right",
				"RC_Hardpoint_V2",
				"RC_Hardpoint_V2_Right",
				"RC_Hardpoint_V3",
				"RC_Hardpoint_V3_Right",
				"RC_Hardpoint_V4",
				"RC_Hardpoint_V4_Right"
			};

			attachment="RC_PylonM_250Rnd_30mm";
		};
		//Left inner
		class pylon3: pylon3
		{
			hardpoints[]=
			{
				"RC_Hardpoint_FuelTank",

				"RC_Hardpoint_V1",
				"RC_Hardpoint_V1_Left",
				"RC_Hardpoint_V2",
				"RC_Hardpoint_V2_Left",
				"RC_Hardpoint_V3",
				"RC_Hardpoint_V3_Left",
				"RC_Hardpoint_V4",
				"RC_Hardpoint_V4_Left"
			};

			attachment="RC_PylonM_19Rnd_Hydra_MP_Proxy";
		};
		//Left outer
		class pylon4: pylon4
		{
			hardpoints[]=
			{
				"RC_Hardpoint_FuelTank",

				"RC_Hardpoint_V1",
				"RC_Hardpoint_V1_Left",
				"RC_Hardpoint_V2",
				"RC_Hardpoint_V2_Left",
				"RC_Hardpoint_V3",
				"RC_Hardpoint_V3_Left",
				"RC_Hardpoint_V4",
				"RC_Hardpoint_V4_Left",
				//"RC_Hardpoint_V5",
				//"RC_Hardpoint_V5_Left",

				//"RC_Hardpoint_V6",
				//"RC_Hardpoint_V6_Left"

				"RC_Hardpoint_V5_Green"
			};

			attachment="RC_PylonM_12Rnd_SRAM_Green";
		};
	};
};
*/


class TransportPylonsComponent: TransportPylonsComponent
{
	class pylons: pylons
	{
		//Right outer
		class pylon1: pylon1
		{
			hardpoints[]=
			{
				"RC_Hardpoint_FuelTank",

				"RC_Hardpoint_V1",
				"RC_Hardpoint_V1_Right",
				"RC_Hardpoint_V2",
				"RC_Hardpoint_V2_Right",
				"RC_Hardpoint_V3",
				"RC_Hardpoint_V3_Right",
				"RC_Hardpoint_V4",
				"RC_Hardpoint_V4_Right",

				"RC_Hardpoint_V5_Green"
			};

			attachment="RC_PylonM_12Rnd_SRAM1_Green";
		};
		//Right inner
		class pylon2: pylon2
		{
			hardpoints[]=
			{
				"RC_Hardpoint_FuelTank",

				"RC_Hardpoint_V1",
				"RC_Hardpoint_V1_Right",
				"RC_Hardpoint_V2",
				"RC_Hardpoint_V2_Right",
				"RC_Hardpoint_V3",
				"RC_Hardpoint_V3_Right",
				"RC_Hardpoint_V4",
				"RC_Hardpoint_V4_Right",

				"RC_Hardpoint_V5_Green"
			};

			//attachment="RC_PylonM_12Rnd_SRAM2_Green";
			attachment="";
		};
		//Left inner
		class pylon3: pylon3
		{
			hardpoints[]=
			{
				"RC_Hardpoint_FuelTank",

				"RC_Hardpoint_V1",
				"RC_Hardpoint_V1_Left",
				"RC_Hardpoint_V2",
				"RC_Hardpoint_V2_Left",
				"RC_Hardpoint_V3",
				"RC_Hardpoint_V3_Left",
				"RC_Hardpoint_V4",
				"RC_Hardpoint_V4_Left",

				"RC_Hardpoint_V5_Green"
			};

			//attachment="RC_PylonM_12Rnd_SRAM3_Green";
			attachment="";
		};
		//Left outer
		class pylon4: pylon4
		{
			hardpoints[]=
			{
				"RC_Hardpoint_FuelTank",

				"RC_Hardpoint_V1",
				"RC_Hardpoint_V1_Left",
				"RC_Hardpoint_V2",
				"RC_Hardpoint_V2_Left",
				"RC_Hardpoint_V3",
				"RC_Hardpoint_V3_Left",
				"RC_Hardpoint_V4",
				"RC_Hardpoint_V4_Left",

				"RC_Hardpoint_V5_Green"
			};

			//attachment="RC_PylonM_12Rnd_SRAM4_Green";
			attachment="";
		};
	};
};