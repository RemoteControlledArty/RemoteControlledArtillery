...
private _usedAngle = (abs (_highAngleSol - _realElevation)) min (abs (_mediumAngleSol - _realElevation));
...
if (_lockedGuided) then {
	if ((_realElevation < (_mediumAngleSol + 6)) and (_realElevation > (_mediumAngleSol - 6))) then {_ctrlElevation ctrlSetTextColor [0,1,0,1];};
